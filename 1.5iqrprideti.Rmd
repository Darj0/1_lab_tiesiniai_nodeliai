...
# Funkcija, kuri patikrina išskirtis pagal IQR (skaičiuoja)
...
# Funkcija, kuri grąžina loginį vektorių (TRUE = išskirtinė reikšmė)
# Naudojame tik duomenų valymui (coef nustatysime 1.5)
is_outlier <- function(x, coef = 1.5) { # Nustatome 1.5 kaip numatytąją (default) reikšmę
  Q1 <- quantile(x, 0.25, na.rm = TRUE)
  Q3 <- quantile(x, 0.75, na.rm = TRUE)
  IQR <- Q3 - Q1
  lower <- Q1 - coef * IQR
...
# Lentelė su išskirčių skaičiumi (analizei, paliekame 1.5 IQR ir 3 IQR)
...
# Duomenų valymas: Pašalinam visas standartines išskirtis (1.5*IQR)
# Nereikia nurodyti 'coef = 1.5', nes tai jau numatytoji reikšmė (default) funkcijoje is_outlier
outlier_list <- lapply(data[numeric_vars], is_outlier) 
rows_to_remove <- Reduce("|", outlier_list)   
data <- data[!rows_to_remove, ] 

cat("Pašalinta išskirčių (1.5 IQR):", sum(rows_to_remove), "eilučių\n")
