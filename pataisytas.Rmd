...
brand_levels <- c(top_brands, "Kiti")

data <- data %>%
  mutate(Markė_Top = ifelse(Markė %in% top_brands, Markė, "Kiti"),
         Markė_Top = factor(Markė_Top, levels = brand_levels))

p1 <- ggplot(data, aes(x = Markė_Top, y = `CO2 emisijos (g/km)`, fill = Markė_Top)) +
  geom_boxplot(color = "black") +
  scale_fill_manual(values = c(rainbow_hcl(length(top_brands), c = 60, l = 80), "grey70")) +
  theme_minimal() +
  labs(title = "CO2 emisijos pagal Top 10 markių",
       x = "Markė",
       y = "CO2 emisijos (g/km)") +
  theme(plot.title = element_text(hjust = 0.5, face = "bold"),
...
freq_marke <- freq_table_df(data, "Markė_Top")
...
# 1. Markė 
p_mean_marke <- mean_point_plot(data, "Markė_Top",
                                color = "navy",
