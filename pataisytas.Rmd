...
```{r}
# --- Train/Test padalijimas ir vertinimas ---
set.seed(2025)
...
train <- droplevels(data[ix, ])
test  <- droplevels(data[-ix, ])

# Svarbu: faktorių lygiai turi sutapti
train$`Pavarų dėžės tipas` <- factor(train$`Pavarų dėžės tipas`)
test$`Pavarų dėžės tipas`  <- factor(test$`Pavarų dėžės tipas`, levels = levels(train$`Pavarų dėžės tipas`))

train$`Kuro tipas` <- factor(train$`Kuro tipas`)
test$`Kuro tipas`  <- factor(test$`Kuro tipas`, levels = levels(train$`Kuro tipas`))

train$`Kėbulo tipas` <- factor(train$`Kėbulo tipas`)
test$`Kėbulo tipas`  <- factor(test$`Kėbulo tipas`, levels = levels(train$`Kėbulo tipas`))

train$`Markė` <- factor(train$`Markė`)
test$`Markė`  <- factor(test$`Markė`, levels = levels(train$`Markė`))

# Pasirink savo geriausią specifikaciją; čia pavyzdys su log(y)
m_train <- lm(log(`CO2 emisijos (g/km)`) ~ 
                `Vidutinės kuro sąnaudos (l/100km)` + 
                `Variklio tūris (l)` +
                `Pavarų dėžės tipas` + 
                `Kuro tipas` + 
...
# Prognozė test rinkinyje ir metrikos (grįžtam į g/km)
pred_log <- predict(m_train, newdata = test)
pred <- exp(pred_log)
y <- test$`CO2 emisijos (g/km)`

RMSE <- sqrt(mean((pred - y)^2))
MAE  <- mean(abs(pred - y))
R2_out <- cor(pred, y, use = "complete.obs")^2

c(RMSE = RMSE, MAE = MAE, R2_out = R2_out)
