...
data <- data %>%
  filter(`Kuro tipas` != "Etanolis (E85)")
...
train_data[c(648, 351), ]

outliers_to_remove <- c(648, 351)
...
648 stebėjimas — beveik garantuota, kad duomenų neatitikimas (klaida), nes CO₂ emisijos per mažos lyginant su sąnaudomis.

351 — gali būti ekologiškai prastas, bet realus automobilis (įtakingas taškas)

648 ir 351 stebėjimai buvo pašalinti siekiant pagerinti modelio stabilumą.
...
Kuro sąnaudos mieste × Pavarų dėžės tipas
...
```
Palyginus modelius, gauta F = 5.829 ir p = 3.843e-14.
...
vidutinė absoliuti paklaida (MAE) – 1.5,
o determinacijos koeficientas (R²) yra 0.994.
